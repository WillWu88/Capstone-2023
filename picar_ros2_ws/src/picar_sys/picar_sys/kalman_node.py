...
class KalmanNode(Node):
    ...
    def __init__(self):
        ...
        self.kf_init(self.kf_x, self.f_s, np.array([[0., 1.]]), ms_cov * R_mult_x, 
        ...
        self.kf_init(self.kf_y, self.f_s, np.array([[0., 1.]]), gps_cov * R_mult_y,
                     Q_mult_y * Q_discrete_white_noise(2., 1./self.f_s, var=imu_y_var))

        # yaw kf
        self.kf_psi = KalmanFilter(dim_x=1, dim_z=1)
        self.kf_init(self.kf_psi, self.f_s, np.array([[1.]]), tan_cov * R_mult_psi,
                     Q_mult_psi * imu_psi_var)
